...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        if j < 7:
            neighbours.add((i,j+1))
        #making sure known cells arent added into the system
        ...
        for sentence in self.knowledge:
            if len(sentence.cells) == 0:
                to_be_removeed.append(sentence)
        for i in to_be_removeed:
        ...
        for sentence in self.knowledge:
            ...
            if safes:
                for safe in safes:
                    temp_safe.append(safe)
        for mine in temp_mine:
            self.mark_mine(mine)
        for safe in temp_safe:
            self.mark_safe(safe)
        #5
        for sentence1 in self.knowledge:
            for sentence2 in self.knowledge:
                if sentence1.cells.issubset(sentence2.cells):
                    ...
                    temp_sen = Sentence(temp_cells,temp_count)
                    mines = temp_sen.known_mines()
                    safes = temp_sen.known_safes()
                    if mines:
                        for mine in mines:
                            self.mark_mine(mine)
                    if safes:
